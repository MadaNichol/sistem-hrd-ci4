...
<div class="flex min-h-screen">
  ...
  <div>
    <div class="text-center mb-8">
      <img src="<?= base_url('img/kT.jpg') ?>" alt="Logo" class="mx-auto mb-4 rounded-full" width="100" height="100">
    ...
    <div class="flex-1 p-4 bg-white">
        ...
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="/auth/tambahcalonkaryawan" class="inline-block bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">
                Tambah Calon Karyawan
            </a>

            <!-- Kolom Pencarian -->
            <input type="text" id="searchInput" placeholder="Cari nama calon karyawan"
            ...
            <table class="w-full table-auto">
                <thead class="bg-gray-100 border-b">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-center text-sm font-semibold">Nama Calon Karyawan</th>
                        <th class="px-4 py-2 text-center text-sm font-semibold">Alamat</th>
                        <th class="px-4 py-2 text-center text-sm font-semibold">No Telp</th>
                        <th class="px-4 py-2 text-center text-sm font-semibold">Jabatan Yang Dipilih</th>
                        <th class="px-4 py-2 text-center text-sm font-semibold">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                <?php foreach ($calon_karyawan as $calon_karyawan): ?>                        
                            <tr class="border-b hover:bg-gray-50">
                            <td class="px-4 py-2 text-center text-sm"><?= $calon_karyawan['id_calonkaryawan'] ?></td>
                            <td class="px-4 py-2 text-center text-sm"><?= $calon_karyawan['nama_calonkaryawan'] ?></td>
                            <td class="px-4 py-2 text-center text-sm"><?= $calon_karyawan['alamat'] ?></td>
                            <td class="px-4 py-2 text-center text-sm"><?= $calon_karyawan['no_telp'] ?></td>
                            <td class="px-4 py-2 text-center text-sm"><?= $calon_karyawan['nama_jabatan'] ?></td>
                            <td class="px-4 py-2 text-center text-sm space-x-2">
                                <!-- Edit -->
                                <a href="/auth/editcalonkaryawan/<?= $calon_karyawan['id_calonkaryawan'] ?>"
                                    class="text-yellow-500 hover:text-yellow-600" title="Edit">
                                    <i class="fas fa-edit text-lg"></i>
                                </a>

                                <!-- Delete -->
                                <a href="/deletecalonkaryawan/<?= $calon_karyawan['id_calonkaryawan'] ?>"
