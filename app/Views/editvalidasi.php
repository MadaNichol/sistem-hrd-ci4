...
<head>
    ...
    <title>Edit Validasi Kontrak</title>
    <link href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css" rel="stylesheet">
</head>
<body class="bg-gray-100 min-h-screen flex items-center justify-center">
    <div class="w-full max-w-xl bg-white p-8 rounded-lg shadow-md">
        <h1 class="text-2xl font-bold text-center text-gray-800 mb-6">Edit Validasi Kontrak</h1>

        <form action="/auth/updatevalidasi/<?= $kontrakk['id_kontrak'] ?>" method="post" class="space-y-4">
            <div>
                ...
                <input type="text" id="id_kontrak" name="id_kontrak" value="<?= $kontrakk['id_kontrak'] ?>" required
                       class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring focus:ring-blue-300" readonly>
            </div>
            ...
            <div>
                ...
                <select name="status" id="status"
                    ...
                    <option value="pending" <?= $kontrakk['status'] == 'pending' ? 'selected' : '' ?>>Pending</option>
                    <option value="disetujui" <?= $kontrakk['status'] == 'disetujui' ? 'selected' : '' ?>>Setuju</option>
                    <option value="ditolak" <?= $kontrakk['status'] == 'ditolak' ? 'selected' : '' ?>>Tolak</option>
                </select>
            </div>

            <div class="flex justify-between items-center">
                <a href="/validasikontrak"
