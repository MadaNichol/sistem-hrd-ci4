...
<div class="flex min-h-screen">
  ...
  <div class="flex-1 p-4 bg-white">
    <h1 class="text-2xl font-bold mb-4">Jabatan</h1>
    <p class="mb-8">Welcome, Admin HRD</p>

    <div class="flex justify-between items-center mb-4">
      <!-- Tombol Tambah Jabatan -->
      <a href="/auth/tambahjabatan" class="inline-block bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">
        Tambah Jabatan
      </a>
    ...
    <div class="w-full bg-white shadow rounded-lg overflow-x-auto">
      <table class="w-full table-auto">
        ...
        <tr>
          <th class="px-4 py-2 text-center text-sm font-semibold text-gray-700">ID Jabatan</th>
        ...
        <?php foreach ($jabatan as $jabatan): ?>
          <tr class="bg-gray-50 border-b">
            <td class="px-4 py-2 text-center text-sm text-gray-600"><?= $jabatan['id_jabatan'] ?></td>
            <td class="px-4 py-2 text-center text-sm text-gray-600"><?= $jabatan['nama_jabatan'] ?></td>
            <td class="px-4 py-2 text-center text-sm space-x-2">
              <!-- Edit -->
              <a href="/auth/editjabatan/<?= $jabatan['id_jabatan'] ?>"
                class="text-yellow-500 hover:text-yellow-600" title="Edit">
                <i class="fas fa-edit text-lg"></i>
              </a>

              <!-- Delete -->
              <a href="/deletejabatan/<?= $jabatan['id_jabatan'] ?>"
...
<script>
  ...
  searchInput.addEventListener("keyup", function () {
    ...
    tableRows.forEach(row => {
      // Cari berdasarkan kolom 2 (jenis jabatan) misal
      const jenisJabatan = row.querySelector("td:nth-child(2)").textContent.toLowerCase();

      if (jenisJabatan.includes(query)) {
        row.style.display = "";
      } else {
